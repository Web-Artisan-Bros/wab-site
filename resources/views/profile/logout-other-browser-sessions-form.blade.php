<x-jet-action-section>
    ...
    <x-slot name="content">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            <div class="text-sm text-gray-600">
                                {{ $session->agent->platform() ? $session->agent->platform() : trans('Unknown') }} - {{ $session->agent->browser() ? $session->agent->browser() : trans('Unknown') }}
                            </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ trans('This device') }}</span>
                                    @else
                                        {{ trans('Last active') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ trans('Please enter your password to confirm you would like to log out of your other browser sessions across all of your devices.') }}
