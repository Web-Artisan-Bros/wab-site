@if(!$formSent)
  <form wire:submit.prevent="store" method="POST">
    ...
    <fieldset>
      <div class="form-floating mb-3">
        <input type="text"
               ...
               id="nameFormInput"
               placeholder="{{ __('forms.name_placeholder') }}" wire:model="name">
        @error('name')
      ...
      <div class="form-floating mb-3">
        <input type="email"
               ...
               id="emailFormInput" placeholder="{{ __('forms.email_placeholder') }}"
               wire:model="email">
        @error('email')
        <div class="invalid-feedback">{{ $message }}</div> @enderror
      ...
      <div class="form-floating mb-3">
        <input type="text"
               ...
               id="phoneFormInput" placeholder="{{ __('forms.phone_placeholder') }}"
               wire:model="phone">
        @error('phone')
        <div class="invalid-feedback">{{ $message }}</div> @enderror

        <label for="phoneFormInput" class="form-label">{{ __('forms.phone') }}</label>
      </div>

      <div class="form-floating mb-3">
      <textarea class="form-control @error('message') is-invalid @enderror"
                id="messageFormInput" rows="3"
                placeholder="{{ __('forms.message_placeholder') }}"
                wire:model="message"
    ...
    <p class="accent-color mb-5 ms-3 mt-2">* {{ __('forms.required_field_msg') }}</p>
